...
- [x] **Step 3: Write `server/src/jsonl-status.ts`**
...
- [x] **Step 1: Write tests for `extractEnrichment`**
...
- [x] **Step 3: Write `server/src/watcher.ts`**
...
- [x] **Step 1: Update `server/src/relay.ts` — expose `broadcastRaw`**
...
- [x] **Step 2: Run relay tests — confirm no breakage**
...
- [x] **Step 3: Update `server/src/index.ts`**
...
- [x] **Step 1: Add `uninstallHooks` tests**
...
- [x] **Step 2: Run tests — verify the new tests fail**
...
- [x] **Step 3: Add `uninstallHooks` to `desktop/src/installer.ts`**
...
- [x] **Step 1: Create `desktop/src/preload.ts`**
...
- [x] **Step 2: Create `desktop/assets/hooks-setup.html`**
...
- [x] **Step 3: Create `desktop/assets/hooks-remove.html`**
...
- [x] **Step 4: Update `desktop/electron-builder.yml`**
...
- [x] **Step 5: Replace `desktop/src/main.ts`**
...
- [x] **Step 7: Manually verify tray menu in dev mode**
...
- [x] **Step 1: Replace `client/src/types.ts`**
...
- [x] **Step 2: Run client tests — confirm no breakage from type changes**
...
- [x] **Step 1: Replace `client/src/hooks/useRelay.ts`**
...
- [x] **Step 1: Write failing tests for new reducer actions**
...
- [x] **Step 3: Replace `client/src/reducer.ts`**
...
- [x] **Step 4: Run all client tests — verify they pass**
...
- [x] **Step 1: Create `client/src/model-limits.ts`**
...
- [x] **Step 2: Create `client/src/components/TokenGauge.tsx`**
...
- [x] **Step 1: Replace `client/src/components/AgentNode.tsx`**
...
- [x] **Step 2: Update `client/src/components/ProjectGroup.tsx` — pass `isTeammate` prop**
...
- [x] **Step 3: Run client tests — confirm no regressions**
...
- [x] **Step 1: Add enrichment section to `AgentDetail`**
...
- [x] **Step 1: Create `client/src/components/HooksBanner.tsx`**
...
- [x] **Step 2: Update `client/src/App.tsx`**
...
- [x] **Step 4: Start dev server and manually verify**
...
- [x] `npm test -w server` — all passing (includes model-limits, jsonl-status, watcher, relay, bootstrap, transcript)
- [x] `npm test -w client` — all passing (reducer, including new SESSION_DISCOVERED / ENRICH / waitingSince / hasHooks tests)
- [x] `npm test -w desktop` — all passing (includes new uninstallHooks tests)
- [x] `npm run dev` starts without errors; dashboard loads at `:5173`
- [x] HooksBanner visible when hooks not installed; dismisses and persists via localStorage
- [x] AgentDetail shows token gauge and enrichment section for agents with JSONL data
- [x] Waiting timer counts up correctly when agent status is `waiting`
- [x] Stale dimming (60% opacity) applies after 30min inactivity on idle/done agents
- [x] Teammate badge appears on root agents when 2+ exist in same project
- [x] `npm run dev:desktop` — tray shows "Install hooks" or "Remove hooks" correctly
- [x] Clicking "Install hooks" in tray opens BrowserWindow with hooks-setup.html
- [x] Clicking "Remove hooks" in tray opens BrowserWindow with hooks-remove.html
- [x] hooks-setup "Install hooks" button installs and closes window; tray flips to "Remove hooks"
- [x] hooks-remove "Remove hooks" button removes and closes window; tray flips to "Install hooks"
- [x] `uninstallHooks()` leaves non-The-Office hooks untouched (covered by tests)
